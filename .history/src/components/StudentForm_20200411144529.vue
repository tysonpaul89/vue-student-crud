  ...
  <div class="container">
    <form>
      <div class="form-group">
        ...
        <input type="text" id="name" name="name" v-model.lazy="studentForm.name" />
      </div>
      <div class="form-group">
        <label>Gender</label>
        <input type="radio" name="Male" value="Male" v-model.lazy="studentForm.gender">Male
        <input type="radio" name="Female" value="Female" v-model.lazy="studentForm.gender">Female
      </div>
      <div class="form-group">
        <label for="month">Birth Month</label>
        <select name="month" id="month" v-model.lazy="studentForm.birthMonth">
          <option v-for="(month, index) in months" :key="index" :value="index">{{month}}</option>
        </select>
      </div>
      <div class="form-group">
        <label for="year">Birth Year</label>
        <select name="year" id="year" v-model.lazy="studentForm.birthYear">
          <option v-for="(year, index) in years" :key="index" :value="year">{{year}}</option>
        </select>
      </div>
      <button type="submit" @submit.prevent="onSubmit()">{{ type === 'create' ? 'Add' : 'Edit' }}</button>
